...
func TestLoadIncludesWebhookJobAndDatabasePoolDefaults(t *testing.T) {
	...
	require.Equal(t, 3, cfg.ControlDBMaxOpenConns)
	require.Equal(t, 1, cfg.ControlDBMaxIdleConns)
	require.Equal(t, 4, cfg.SyncDBMaxOpenConns)
	require.Equal(t, 1, cfg.SyncDBMaxIdleConns)
