...
func printRepoChangeStatus(out io.Writer, status RepoChangeStatusResponse) {
	...
	fmt.Fprintf(tw, "Fetch owner:\t%s\n", coalesce(status.FetchLeaseOwnerID, "-"))
	fmt.Fprintf(tw, "Fetch heartbeat:\t%s\n", humanTimePtr(status.FetchLeaseHeartbeatAt))
	fmt.Fprintf(tw, "Fetch lease expires:\t%s\n", humanTimePtr(status.FetchLeaseExpiresAt))
	fmt.Fprintf(tw, "Backfill owner:\t%s\n", coalesce(status.BackfillLeaseOwnerID, "-"))
	fmt.Fprintf(tw, "Backfill heartbeat:\t%s\n", humanTimePtr(status.BackfillLeaseHeartbeatAt))
	fmt.Fprintf(tw, "Backfill lease expires:\t%s\n", humanTimePtr(status.BackfillLeaseExpiresAt))
