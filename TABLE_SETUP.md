...
`ghreplica` keeps its production schema under explicit SQL migrations.
...
This document covers `ghreplica` application tables. Migration bookkeeping
tables such as `schema_migrations`, and third-party tables owned by River, are
managed separately and are not part of the GORM model registry.
