...
This document covers how `ghreplica` tests, coverage checks, and lint-like
checks are expected to work.
...
- package coverage floors for high-risk packages
- `golangci-lint`
...
`ghreplica` uses normal Go `*_test.go` files and the standard `testing`
package.
...
```sh
go test ./internal/httpapi ./internal/webhooks
go test ./internal/githubsync
go test ./cmd/ghreplica ./cmd/ghr
go test ./...
```
...
Bug fixes should normally ship with a regression test that would have caught
...
Some local environments may have live GitHub auth variables set. Those can
affect contract tests that intentionally rely on explicit environment setup.

For a clean repo-wide coverage run, prefer:

```sh
env -u GITHUB_TOKEN -u GH_TOKEN -u GHREPLICA_CONTRACT_REPO \
  go test ./... -coverprofile=cover.out
go tool cover -func=cover.out
```
...
`ghreplica` enforces a minimum total coverage floor in CI through
...
That style is conventional, and it is the model `ghreplica` uses in CI.
...
For `ghreplica`, the goal is not blind `100%` coverage across the whole repo.

Preferred long-term package targets:

- `internal/httpapi`: `85%+`
- `internal/webhooks`: `85%+`
- `internal/githubsync`: `80%+`
- `internal/gitindex`: `80%+`
...
./scripts/check-go-coverage.sh
/home/bob/go/bin/golangci-lint run
go build ./cmd/ghreplica
go build ./cmd/ghr
docker build -t ghreplica:ci .
```
...
`ghreplica` should keep test intent clear.
...
- Integration tests cover real package boundaries such as HTTP handlers,
  database behavior, webhook processing, or client interactions.
...
- An integration test should usually exercise a real boundary such as SQL,
  migrations, HTTP request handling, or webhook/client behavior.
...
Typical valid boundaries in `ghreplica` include:
...
Prefer testing real code paths inside `ghreplica` unless the test is
intentionally isolating an external dependency or boundary.
...
`ghreplica` currently gates on complexity metrics through `golangci-lint`.
...
- Coverage and complexity thresholds should be tightened deliberately over
  time, not raised accidentally by tool churn.
- Slow or environment-heavy tests should not degrade the normal
  `go test ./...` path.
